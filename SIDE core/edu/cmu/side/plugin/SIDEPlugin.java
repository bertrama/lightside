...
public abstract class SIDEPlugin implements XMLable, Cloneable{
	...
	public Object clone(){
		...
		try {
			return createFromXML(XMLBoss.XMLFromString(xmlString).getDocumentElement());
		} catch (Exception e) {
	...
	public static SIDEPlugin createFromXML(Element root){
		...
		try{
			PluginWrapper wrapper = Workbench.current.pluginManager.getPluginWrapperByPluginClassName(className);
