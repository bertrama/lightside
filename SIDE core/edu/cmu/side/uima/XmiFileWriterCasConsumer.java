 ...
 * </ul>
 */
public class XmiFileWriterCasConsumer extends CasConsumer_ImplBase {
  /**
  ...
  public void initialize() throws ResourceInitializationException {
    mFile = new File((String) getConfigParameterValue(PARAM_OUTPUTFILE));
    File outputDir = mFile.getParentFile();
    if (!outputDir.exists()) {
    	outputDir.mkdirs();
    }
  }
   ...
   * In this case, the CAS is converted to XMI and written into the output xmiFile .
   * 
   * @param aCAS
  ...
  public void processCas(CAS aCAS) throws ResourceProcessException {
    ...
    // retreive the filename of the input xmiFile from the CAS
    ...
    // serialize XCAS and write to output xmiFile
    try {
      writeXmi(jcas.getCas(), mFile);
    } catch (IOException e) {
      throw new ResourceProcessException(e);
  ...
  /**
   * Serialize a CAS to a xmiFile in XMI format
   * 
   * @param aCas
   *          CAS to serialize
   * @param name
