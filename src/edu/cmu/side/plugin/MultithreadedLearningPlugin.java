...
public abstract class MultithreadedLearningPlugin extends LearningPlugin implements Serializable
{
					...
					throws Exception
	{
		...
		for (Integer fold : folds)
		{
			...
			if (halt) { throw new Exception("User Canceled"); }
			...
			OrderedPluginMap wrappers, boolean learn)
	{
		for (SIDEPlugin wrapper : wrappers.keySet())
		{
			...
			if (learn)
			{
				...
				// whatever the wrapper has learned is stored in configuration
				// settings.
